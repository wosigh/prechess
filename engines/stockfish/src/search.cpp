...
bool think(const Position& pos, bool infinite, bool ponder, int side_to_move,
  ...
  if (UseTimeManagement && !ponder && get_option_value_bool("OwnBook"))
  {
      ...
      if (bookMove != MOVE_NONE)
      {
          ...
          syslog(LOG_WARNING, "--- stockfish bestmove 1 --- \n");
          return true;
      }
  }
...
namespace {
  ...
  Value id_loop(const Position& pos, Move searchMoves[]) {
    ...
    syslog(LOG_WARNING, "--- stockfish bestmove 2 --- \n");
  ...
  void poll() {
    ...
    if (Bioskey())
    {
		...
		// syslog(LOG_WARNING, "--- stockfish command %c%c%c%c --- \n",command[0],command[1],command[2],command[3]);
        ...
        {
            // syslog(LOG_WARNING, "--- stockfish command stop --- \n");
  			AbortSearch = true;
            PonderSearch = false;
        }
