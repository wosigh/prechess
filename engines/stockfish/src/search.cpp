...
namespace {
  ...
  int NodesSincePoll;
  int NodesBetweenPolls = 3000; // MEME
...
bool think(const Position& pos, bool infinite, bool ponder, int side_to_move,
  ...
  if (/* MEME UseTimeManagement &&  */ get_option_value_bool("OwnBook"))
  {
      // MEME if (get_option_value_string("Book File") != OpeningBook.file_name())
...
#endif
          sprintf(shmout,"bestmove %s\n",move_to_string(bookMove).c_str());
          return true;
      }
  }
  ...
  // heavy time pressure.
  if (MaxNodes)
      NodesBetweenPolls = Min(MaxNodes, 3000); // MEME 30000
  else if (myTime && myTime < 1000)
      NodesBetweenPolls = 1000;
  else if (myTime && myTime < 5000)
      NodesBetweenPolls = 5000;
  else
      NodesBetweenPolls = 3000; // MEME 30000
...
namespace {
  ...
  Value id_loop(const Position& pos, Move searchMoves[]) {
    ...
    if (ss[0].pv[1] != MOVE_NONE)
    {	
    	SYSLOG(LOG_WARNING,"ybestmove %s ponder %s\n"  , move_to_string(ss[0].pv[0]).c_str()	,move_to_string(ss[0].pv[1]).c_str());
    	sprintf(shmout,"bestmove %s ponder %s\n"  , move_to_string(ss[0].pv[0]).c_str()	,move_to_string(ss[0].pv[1]).c_str());
    }
    else
    {
    	sprintf(shmout,"bestmove %s\n"  , move_to_string(ss[0].pv[0]).c_str());
    	SYSLOG(LOG_WARNING,"bestmove %s\n"  , move_to_string(ss[0].pv[0]).c_str());
    } 
        ...
        for (int i = 0; i <  rml.move_count() && !AbortSearch; i++)
        {
            ...
            else
            {
                ...
                if (MultiPV == 1)
                {
					...
					std::string  sss;
					   sss = ""; 
					   sss += value_to_string(value);
						 sss +=	" ";


					for (int j = 0; ss[0].pv[j] != MOVE_NONE && j < PLY_MAX; j++)
					{
						sss += move_to_string(ss[0].pv[j]); 
						sss += " ";
					}   

					sprintf(&shmheartbeat[2],"%s",sss.c_str());
                    if (value > alpha)
                        alpha = value;
                }
                else // MultiPV > 1
...
static long cnt=1000000;
  ...
  void poll() {
    ...
    if (shmin[0]!=0)
    {
    	...
    	command = shmin;
    	SYSLOG(LOG_WARNING,"command=%s",command.c_str());
    	shmin[0]=0;
